...
const PriceAndRemove = ({ price, details, path, title, icon, remove, classDiv, onClickRemove }) => {
  return (
        <div className={"flex flex-col justify-between items-center"+" "+classDiv}>
            {
              remove !== "no" ?
              <Button onClick={onClickRemove} color='red' className='flex items-center gap-1 hover:!shadow-none !p-2 text-sm' variant="text">ازالة<MdOutlineRemoveShoppingCart className="text-sm"/></Button>
              : null
            }
            { details === "yes" ?
            ...
            <div className="flex gap-1 items-center">
                ...
                <p className='!text-sm text-gray-500'>ج.م</p>
            </div>
        </div>
  )
}
