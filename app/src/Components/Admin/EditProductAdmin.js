...
import { Button, Dialog, DialogBody, DialogFooter, DialogHeader, IconButton, Option, Select, Spinner, Tooltip, Typography, select } from '@material-tailwind/react';
...
import EditProductHook from "../../hook/products/edit-product-admin-hook";

const EditProductAdmin = () => {

    const { id } = useParams();

    const [product, handleName, handleDiscription, handleImgState, handleEditProduct, handleBrandSelect,
        ...
        priceBefore, price, NumberOfProducts, open, description, name, loading, press, handleActiveBrand, activeBrand, select, defaultSelect, brandSelect, imgs] = EditProductHook(id);
            ...
            <div className="container mx-auto pb-[8rem]">
                {press ? loading ? <div className="w-full h-full absolute top-0 left-0 flex justify-center items-center bg-opacity-30 bg-black z-50"><Spinner color='gray' className="h-8 w-8 text-gray-900" /></div> : null : null}
                    ...
                    <div className="flex flex-col gap-4 basis-full">
                        <SubTitle title={`تعديل المنتج - ${name}`} classDiv={"py-4 pb-6 !px-0"} />
                        <form onSubmit={handleEditProduct} className="flex flex-col gap-3" method="POST">
                        <div className="flex flex-col gap-3 items-center">
                            {imgState === true ? <span className="pt-4 font-bold text-gray-600">
                                      {imgUrls.length} صور مختارة
                                </span> : null}
                            {imgState === true ?
                            <div className="w-fit h-full rounded-lg overflow-hidden border border-gray-300 flex gap-3 items-center flex-wrap">
                                {imgState === true ? imgUrls.map((imgUrl, index) => {
                                    return (<div key={index} className="relative"><TiDelete onClick={() => removeImg(imgUrl)} color="red" className="cursor-pointer absolute top-2 z-50 left-2" size={20} /><img loading="lazy" className="relative max-h-60 w-44 object-center border-r border-r-gray-300 first:border-r-transparent p-2 backdrop-brightness-50 brightness-50" alt={"img"} src={imgUrl} /></div>);
                                }) : null}
                            </div> : null }
                        {imgState === true ? 
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="flex flex-row items-center gap-4 w-full">
                                <select value={select} onChange={(e)=>handleSelect(e.target.value)} name="cat" className="outline-none focus:outline-none w-full border border-gray-300 rounded-lg p-2 cursor-pointer">
                                    {category.data ? category.data.map((item, index) => {
                                            return <option onClick={(e)=>handleSelect(e.target.value)} value={item._id} className={"flex items-center justify-between bg-transparent cursor-pointer" + " " + (active === index ? "bg-blue-gray-50" : active === -1 ? "" : "" )} key={item._id} >
                                                {item.name}
                                                </option>;
                                            }) : null}
                                </select>
                            ...
                            <div className="flex flex-row items-center gap-4">
                                <select value={brandSelect} onChange={(e) => handleBrandSelect(e.target.value)} name="cat" className="outline-none focus:outline-none w-full border border-gray-300 rounded-lg p-2 cursor-pointer">
                                {brand.data ? brand.data.map((item, index) => {
                                        return <option value={item._id} className={"flex items-center justify-between bg-transparent" + " " + (activeBrand === index ? "bg-blue-gray-50" : activeBrand === -1 ? "" : "" )} key={item._id}>
                                            {item.name}
                                        </option>;
                                    }) : null}
                                </select>
